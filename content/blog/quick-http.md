...
HTTPはアーキテクチャスタイルとしてクライアント・サーバーを採用しています。
...
> [!NOTE]
> Webブラウザのアドレスバーに日本語を含んだURIを書いてアクセスすることができますが、クライアントとサーバー間では%エンコードされた値をやりとりします。
...
リクエストとレスポンスの具体的な中身について見ていきましょう。  
この具体的な中身のことをそれぞれリクエストメッセージとレスポンスメッセージと呼びます。  
さらに、**リクエストメッセージとレスポンスメッセージを総称してHTTPメッセージ**と呼びます。

> [!NOTE]
> 本セクションであげているHTTPメッセージの例は、[HTTP メッセージ - HTTP | MDN](https://developer.mozilla.org/ja/docs/Web/HTTP/Guides/Messages)から引用したものです。

### リクエストメッセージ

リクエストメッセージはたとえば以下のようなものです。
...
リクエストメッセージは**リクエストライン、ヘッダー、空行、ボディの4つの要素**から構成されています。

**リクエストライン**は、HTTPメソッド、パス、HTTPバージョンの3つの情報を含みます。
上記の例では`POST /users HTTP/1.1`がリクエストラインです。
HTTPメソッドは操作の種類を表すもので、`GET`や`POST`、`PUT`、`DELETE`などがあります。

**ヘッダー**は、リクエストに関する追加情報を伝えるためのものです。
上記の例では`Host`、`Content-Type`、`Content-Length`の3つのヘッダーがあります。
`Host`はリクエスト先のホスト名を、`Content-Type`はボディのデータ形式を、`Content-Length`はボディのバイト数を表しています。

**空行**は、ヘッダーとボディの境界を示すためのものです。
ヘッダーの後に必ず空行が入ります。

**ボディ**は、サーバーに送信する実際のデータです。
上記の例では`name=FirstName+LastName&email=bsmth%40example.com`がボディで、フォームデータとしてユーザー名とメールアドレスを送っています。
なお、`GET`リクエストのようにボディを持たないリクエストもあります。

### レスポンスメッセージ

レスポンスメッセージはたとえば以下のようなものです。
